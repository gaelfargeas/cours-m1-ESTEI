...
public class applet_numtel_vector extends Frame implements ActionListener{
	...
	public void search_name(String nom)
	{
		...
		try 
		{
			...
			if(!numtrouver) 
			{
				...
				if (msgbox.affrep(this, "Vouvez vous rajouter un nom"))
				{
					
					newname=tfnom.getText();
					System.out.println("le nouveau nom est " +newname);
					addnumero(inbox.affmsg(this, "saisir un numero"));
	...
	public void addnumero (String newnum) 
	{
		...
		try {
			...
			bw.newLine();
			System.out.println("le nouveau numero est " + newnum);
			bw.close();
			pr.close();
